...
export async function GET(
  ...
  if (!isAdmin && !isParticipant && !isPairParticipant && !(plan.isPublic && isDataspaceMember)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const block = await prisma.planBlock.findFirst({
    where: { id: params.blockId, planId: plan.id, type: "TEXT" }
  });
...
export async function POST(
  ...
  const block = await prisma.planBlock.findFirst({
    where: { id: params.blockId, planId: params.id, type: "TEXT" }
  });
